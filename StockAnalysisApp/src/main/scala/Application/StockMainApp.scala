...
object StockMainApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    /**
     * NOTE: The current implementation works only for serial source
     */
    val sourceKafka: DataStream[Batch] = env.fromSource(kafkaSource, WatermarkStrategy.noWatermarks(), "Kafka Source").setParallelism(1).name("Kafka Source")
    //    sourceKafka.print()


    val eventUnpacked: DataStream[EventUnpackSchema] = sourceKafka.flatMap(new BatchToEventUnpack()).setParallelism(1).name("BatchToEventUnpack")
    //    eventUnpacked.print()
     ...
     */
    val emaWin: DataStream[EMASchema] = processedWindow.keyBy(_.Symbol).map(new EMACalculator(j = param._1, w = param._2)).name("EMA Calculator")
    //    emaWin.print()
  ...
  def parameterParser(arguments: Array[String]) : (Double, Double, Int, String) ={
    ...
    try {
      ...
      println(s"\n\tj1: ${j1}\n\tj2: ${j2}\n\tCheckpoint: ${if (checkpointInterval == -1.0){
          "No Checkpoint"
        }else{
